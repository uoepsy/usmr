...
<small style="display:block;text-align:right">You are viewing an interactive version of this reading.<br>Please allow ~30 seconds for packages to load before code chunks can be run<br><a class="intlink" href="../readings/03_measurement.html">Click here for the static version</a></small>
...
```{webr}
# get the values in the "cracked" variable from the "pwords" dataframe, and sum them all together. Then divide this by 500
...
```
> **translation:** take the data and select the "variable1" and "variable2" columns
...
```
> **translation:** take the data and filter it to only the rows where the "variable1" column is equal to "value1". 
...
# the above code won't print anything
# to check, you might want to show the pwords data:
pwords
```
...
p + geom_area(data = subset(d, x > 120), aes(x=x, y=y), fill="red")
```
