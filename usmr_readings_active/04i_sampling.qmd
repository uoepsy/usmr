...
<small style="display:block;text-align:right">You are viewing an interactive version of this reading.<br>Please allow ~30 seconds for packages to load before code chunks can be run<br><a class="intlink" href="../readings/04_sampling.html">Click here for the static version</a></small>
...
In [the previous Chapter](03_measurement.html#histograms) we started to visualise sample distributions that looked similar to this (i.e. roughly bell-shaped), and we saw how the shape of such distributions depends on parameters such as the mean and variance/standard deviation^[remember that standard deviation is $\sqrt{\text{variance}}$] (see @fig-bellsd). 
```{r}
#| label: fig-bellsd
...
Let's return to the IQ data we saw in [Chapter 3 #histograms](03_measurement.html#histograms). We have our observed sample distribution of IQ scores: 
...
```{webr}
# our sample:
wechsler <- read.csv("https://uoepsy.github.io/data/wechsler.csv")
...
Note that the mean of our sample is not quite the same as the mean of the population (65 exactly). As we know, *samples vary*. If we do the same thing again, R will take a different sample of 30, and so the mean of this new sample will also be different:  
```{webr}
