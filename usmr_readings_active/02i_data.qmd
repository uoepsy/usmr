...
<small style="display:block;text-align:right">You are viewing an interactive version of this reading.<br>Please allow ~30 seconds for packages to load before code chunks can be run<br><a class="intlink" href="../readings/02_data.html">Click here for the static version</a></small>
...
- If you clear your environment, restart R, and run your script all at once (see the section on ['Good Habits' in Chapter 1](01_R.html#Good_Habits){target="_blank"}), it shouldn't produce errors and should get you back to your current place  
...
Thinking back to Chapter 1 when we first introduced R, we [talked briefly about "logical operators"](01_R.html#R_is_a_calculator){target="_blank"}. Specifically, the operators `&`, `|`, and `!` (for "and", "or"," and "not"), will come in handy now.  
...
```
> **translation:** for the row(s) where the name variable in starwars2
is equal to "C-3PO", in the species variable we assign that entry to be "Droid"
