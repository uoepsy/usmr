...
library(tidyverse)
library(patchwork)
recalldata <- read.csv('https://uoepsy.github.io/data/recalldata.csv')
set.seed(017)
```
...
<small style="display:block;text-align:right">You are viewing an interactive version of this reading.<br>Please allow ~30 seconds for packages to load before code chunks can be run<br><a class="intlink" href="../readings/09_covcor.html">Click here for the static version</a></small>
...
pp1 <- ggplot(df2,aes(x=x,y=y))+
  ...
  theme_classic()+
  geom_vline(aes(xintercept=mean(x)), lty="dashed")+
  annotate("text",x=mean(df2$x)+1.5, y=max(df2$y)-5,label="bar(x)", parse = TRUE)+
  geom_hline(aes(yintercept=mean(y)), lty="dashed")+
  annotate("text",x=min(df2$x)+5, y=mean(df2$y)+1,label="bar(y)", parse = TRUE)+
