...
<small style="display:block;text-align:right">You are viewing an interactive version of this reading.<br>Please allow ~30 seconds for packages to load before code chunks can be run<br><a class="intlink" href="../readings/06_inference2.html">Click here for the static version</a></small>
...
In [Chapter 5](05_inference.html) we learned about the logic of _Null Hypothesis Significance Testing_ (NHST), allowing us to draw perform inferentials tests about parameters in the __population__, based on statistics computed on the __sample__ that we have collected.  
...
# the patchwork package is a really handy way to combine plots 
# side by side by just "adding" them to one another
# install it and check it out! 
library(patchwork)
data <- tibble(mysample = mysample)

p1 <- ggplot(data,aes(x=mysample))+geom_histogram(bins=14)

p2 <- ggplot(data,aes(x=mysample))+geom_density()
...
The theoretical quantiles are the equivalent quantiles of the _standard normal distribution_ (see [Chapter 4 #standard-normal-distribution](04_sampling.html#the-standard-normal-distribution){target="_blank"}).  
...
```{webr}
#| autorun: true
wpmtime <- read.csv("https://uoepsy.github.io/data/usmr_tread.csv")
head(wpmtime)
```
```{r}
#| include: false
...
```{webr}
#| autorun: true
tcaff <- read.csv("https://uoepsy.github.io/data/usmr_tcaff.csv")
head(tcaff)
```
:::
...
```{webr}
#| autorun: true
acedata <- read.csv("https://uoepsy.github.io/data/acedata.csv")
head(acedata)
```
