...
[being taught in a _state_ school `r ri(cf(3))` your predicted reading age by `r abs(cf(3))` years]{.fragment} [_if you are taught using the word method_]{.fragment}
...
reading age = `r cf(1)` - $\frac{1}{2}$·`r cf(2)` - $\frac{1}{2}$·`r cf(3)` + $\frac{1}{4}$·`r cf(4)` 

reading age = `r cf(1)` `r pm(-5*cf(2))` `r abs(-.5*cf(2))` `r pm(-.5*cf(3))` `r abs(-.5*cf(3))` `r pm(.25*cf(4))` `r abs(.25*cf(4))` = **`r cf(1)-.5*cf(2)-.5*cf(3)+.25*cf(4)` years**
