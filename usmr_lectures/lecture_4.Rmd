...
statsClasses <- read_csv("https://uoepsy.github.io/data/surveydata_allcourse22.csv")
```
...
## Where were students during COVID19?
]
...
- data from all of the stats modules in Psychology in September 2020.  

```{r tab1}
statsClasses %>% filter(year == 2020) %>%   select(course,in_uk) %>% table()
statsClasses %>% filter(year == 2020) %>%
  select(course,in_uk) %>% table() %>% addmargins()
```
...
.pull-left[
```{r tab5}
t <- statsClasses %>% filter(year == 2020) %>%
