...
statsClasses <- read.csv("https://uoepsy.github.io/data/surveydata_historical.csv")
...
## Where are You From? {background-image="img/playmo_world.jpg" background-opacity=.3}
...
::: {.column width="60%"}
- students on each module would be _equally likely_ to come from the UK

- in other words, of the `r rowSums(table(sc))[1]` students on dapr1, $\frac{`r colSums(table(sc))[2]`}{`r sum(table(sc))`}\times{}`r rowSums(table(sc))[1]`$, or approx `r round(rowSums(table(sc))[1]*colSums(table(sc))[2]/sum(table(sc)),2)` students, should come from the UK under H~0~
:::
...
```{r}
#| warning: false
chisq.test(table(uc),p=c(.09,.75,.02,.03,.05,.06))
```
