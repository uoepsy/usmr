...
rm(newClasses)
newClasses <- read.csv("https://uoepsy.github.io/data/usmr2025.csv") |> select(course,year,eyecolour) |> mutate(in_uk=NA)
statsClasses <- statsClasses |> bind_rows(newClasses)
rm(newClasses)

usmrEyes <- statsClasses |> filter(course=="usmr", year==2025) |> pull(eyecolour) |> discard(is.na)
```
...
ggplot(as.data.frame(eyes), aes(usmrEyes, Freq, fill = usmrEyes)) +
  ...
  scale_fill_manual(values = c('#FFBF00','#218BB2', '#593520', '#7CA69A',
                               '#8C8C8C', 
                               '#5C643D', '#071726')) +
