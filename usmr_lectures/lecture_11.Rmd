...
---
# Coefficients describe the line
...
---
# We can test the coefficients

.pull-left[
```{r echo=FALSE, fig.asp=.8}
m = lm(y~x1,df)
sims = as.data.frame(MASS::mvrnorm(2e3, mu=c(coef(m)[1],0),Sigma=vcov(m)))
names(sims) = c("i","s")
...
.pull-right[
$$y_i = b_0 + \color{red}{b_1}(x_{1i}) + \varepsilon_i$$
...
Plausible range of values for $b_1$: 
```{r eval=FALSE}
confint(model)
```
...
ggplot(., aes(x = x1, y = y)) + 
  ...
  geom_segment(aes(x=x1,xend=x1,y=mean(df$y),yend=y),lty="dotted",lwd=1) +
  labs(title="SS Total") +
...
ggplot(., aes(x = x1, y = y)) + 
  ...
  geom_smooth(method=lm, se = FALSE, fullrange = TRUE) +
  geom_abline(intercept=mean(df$y),slope=0,col="blue",lwd=1)+
  geom_segment(aes(x=x1,xend=x1,y=mean(df$y),yend=.fitted),lty="dotted",lwd=1)+labs(title="SS Model") +
  
broom::augment(m) %>%
ggplot(., aes(x = x1, y = y)) + 
  geom_point(size=2) +
  geom_smooth(method=lm, se = FALSE, fullrange = TRUE) +
  geom_segment(aes(x=x1,xend=x1,y=.fitted,yend=y),lty="dotted",lwd=1)+
