...
- we'll learn how we can make prettier graphs using `ggplot()`
]]
...
p <- t %>% ggplot(aes(x=x)) +
  stat_function(fun=dnorm, n=76, args=list(mean=mean(hData), sd=sd(hData)), size=1.5, colour="darkgrey") +
...
se=sd(hData)/sqrt(n)
p2 <- p +
  stat_function(fun=dnorm, n=1e3, args=list(mean=mean(hData), sd=se), size=1.5)
p2
...
fillme <- layer_data(p2,2) %>% 
  filter(x >= mean(hData)-1.96*se & x <= mean(hData)+1.96*se)
