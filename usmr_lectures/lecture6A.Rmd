...
```{r}
library(tidyverse)
demo <- read_csv("../../data/surveydata_allcourse22.csv") %>%
  filter(course=="usmr") %>%
...
p2 <- ggplot(demo, aes(x = eyecolour, fill= eyecolour)) +
  ...
  guides(fill = "none")+
  labs(title="Student Eye-Colours")
...
```{r}
#| echo: false
m_heights <- replicate(1e4, mean(rnorm(n = 227, mean = 168, sd = 9.1)))
p_breyes <- replicate(1e4, mean(sample(c(0,1), size = 227, replace = TRUE, prob = c(.48, .52))))
par(mfrow=c(2,1))
hist(m_heights, xlab = "Mean heights from samples of n=227",main="")
hist(p_breyes, xlab = "Proportions of brown eyes in samples of n=227",main="")
par(mfrow=c(1,1))
```
]
...
set.seed(67)
sim <- tibble(
  meanheight = replicate(1000, mean(rnorm(n = 227, mean = 168, sd = 9.1))),
  propbrown = replicate(1000, mean(sample(c(0,1), size = 227, replace = TRUE, prob = c(.48, .52)))),
)
...
```{r}
mheights <- replicate(1000, mean(rnorm(n = 227, mean = 168, sd = 9.1)))
sd(mheights)
```
...
---
# is the signal stronger than the noise?

e.g. "Given the signals we expect, how unlikely is the signal we observe?"  

.pull-left[
__Signals we expect:__  
```{r}
#| echo: false
#| fig-height: 5
...
2. how unlikely is the statistic that we observe (the 'signal') given the variation (noise) we would expect?  
...
$$t = \frac{\bar{x}_1 - \bar{x}_2}{SE_{\bar{x}_1-\bar{x}_2}} \, \, = \, \,\frac{(\bar{x}_1 - \bar{x}_2) - 0}{SE_{\bar{x}_1-\bar{x}_2}}$$  
...
---
class: inverse, center, middle
# Week 4 & 5 - more signals, more noise

---
# statistical tests: e.g. chisquare tests
