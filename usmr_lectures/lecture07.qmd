...
::: {.fragment}
- `I()` _isolates_ `+`, `-`, `*`, etc. from the formula context
:::
...
::: aside
`I(scale(...))` isn't needed because there's no ambiguity
:::
...
## Linear Model Using `type` {visibility="unnumbered"}
...
- the intercept is `playmo` _by inference_ from the coefficient
...
toys |> ggplot(aes(x=type,y=UTILITY)) +
  geom_point(size=3) +
  geom_smooth(aes(x=is_zing+1),method="lm")
```
...
toys <- toys |> select(-is_zing) |> full_join(t2)
toys <- toys |> group_by(type) |> mutate(id=1:n()) |> ungroup() |> arrange(id,type) |> select(-id) |> mutate(type=as_factor(type))
```
...
::: aside
I'm keeping these in alphabetic order to match `R` defaults
::: 
...
$$\textrm{utility}_i=\color{blue}{b_0}\color{red}{+b_1\cdot\textrm{is_playmo}_i}+\color{gray}{b_2\cdot\textrm{is_zing}_i}+\epsilon_i$$
...
$$\textrm{utility}_i=\color{blue}{b_0}\color{gray}{+b_1\cdot\textrm{is_playmo}_i}+\color{red}{b_2\cdot\textrm{is_zing}_i}+\epsilon_i$$
...
## [But You Never Compared Playmo to Zing]{.r-fit-text}
...
```{r}
#| echo: false
#| output-line-numbers: "6,12"
toys <-
  toys |> mutate(type=fct_relevel(type,c('playmo','lego','zing')))
mod.r <- lm(UTILITY~type, data=toys)
.pp(summary(mod.r),l=list(0,9:19))
```

## Different Contrast Codings

- multiple models increase the chance of a **type 1** error
- **different contrast coding**s may suit our research question
- a predictor with $g$ levels (or 'groups') has $g-1$ possible contrasts
- contrasts can be _anything you like_ (a few are built in to `R`)
...
::: aside
model fit is not affected, for type I sums-of-squares
:::
...
```{r}
#| include: false
toys <- toys |> filter(type != 'lego') |> droplevels()
```
...
- 'average' (grand mean) utility = `mean(toys$UTILITY)` = `r mean(toys$UTILITY)`
...
#| echo: false
mod.s <- lm(UTILITY ~ type, data=toys)
.pp(summary(mod.s),l=list(0,9:12,0))

```

## Sum Contrasts {visibility="uncounted"}
...
- mean = `r coef(mod.s)[1]`
- difference = `r coef(mod.s)[2]`

::: {.content-visible when-profile="local"}

## [Group Assessment Available on Thursday]{.red .r-fit-text}

:::: {.columns}

::: {.column width="50%"}
- coursework will be released [on Learn]{.red} after labs

- full instructions (and plenty of extra help) online

- please read carefully

- plan time to work with your group
:::

::: {.column width="50%"}
![](img/playmo_group.jpg){width="50%" fig-align="center"}
:::
