...
???
before we get onto the generalized model, i'd like to just tidy up the example we've been using the last couple of weeks in the lecture. 
...
???
- yes indeed, we're still learning to read! 
- so last week we saw a couple of things
- we mainly looked at how reading age is influenced by hours week of practice, and that this is different dependign on whether you learn in the phonics method or the word method
...
.pull-left[
![](lecture_10_files/img/playmo_teach.jpg)
]
.pull-right[
...
???
- that was when we saw this plot here. 
- we can see that practice has a beneficial effect for phonics learners
- but not for word learners
...
???
- we also looked briefly at how we can allow a continuous variable like hrs week practice
- to interact with another continuous variable like age.  
...
???
- and that was where we saw this more general idea of an interaction being a twisting surface 
- i.e. non-parallel lines, but where we have infinitely many lines, making that surface
...
$$\hat{y}_i=b_0+b_1x_{1i}+b_2x_{2i}+\ldots{}+b_kx_{ki}+\ldots{}+b_mx_{1i}x_{2i}+b_{m+1}x_{2i}x_{3i}+\ldots$$
]
...
???
- we've seen in the labs a reasonable number of models where we have lots of predictors.
- in the lectures so far we've just been working with 2 i think
- so it's worth mentioning here that we can easily add predictors to a model by simply estimating more coefficients.
- so with k predictors, we have k coefficients.  
- and we can add in our interactions.  
- and that's kind of all the stuff we've been looking at last week, in the one model.  
  - age, hrs week, and method
  - interaction between hrs week and method, and between hrs week and age

--

- NB., order of predictors **can** matter (judgement is important)
    - if we conduct `anova(mod.mm)` we test _incremental addition_ of each predictor

- first question:  is it worth it building such a complex model?

???
- and remember, the order *might* matter here, depending upon what we do.
- things like the type 1 SS anova, test the variance explained by each predictor when we add them incrementally, one after the other.  
- so in order to tidy up this example, the question i'd like to look at is whether this model is just neccessary?  
...
```
???
- so we can see from this that the age and hrs_wk interaction doesn't improve the model any further than the model without it.
...
???
- here are the assumption plots of our final model
- we can see a fairly straight line in both the left hand plots, which is good
- the QQ plot looks okay, with slight deviations at the bottom there
...
summary(mod.mm)
```
- [coef_as_pred.R](coef_as_pred.R)
???
- so lets look at our model summary
- and i think interpreting all these coefs in speech is actually quite difficult to follow. 
- so [RSTUDIO]
...
???
- let's talk about how we might visualise this model
- well, we're never going to visualise the whole thing. 
- 3d plots are fine, but we don't really want them, and besides, we're already working in more than 3 dimensions. 
- now age doesn't interact, and it's something we've been treating more as a covariate 
  - i.e. something we expect to be associated with reading age, but are not specifically interested in estimating
- so we could plot the thing we _are_ interested in, i.e. this interaction, predicted at the mean age.  
- and that's what things like plot_model do for us
...
.pull-right[
```{r anova1,highlight.output=12, eval=FALSE}
summary(mod.b)
```
```{r dev,echo=FALSE}
...
```{r l2pex2a,results ="asis",echo=FALSE}
cat("```r\n")
cat(paste0("l2p(",a,"+",b,"*10)\n"))
cat("```\n")
```
```{r l2pexr2a,echo=FALSE}
l2p(a+10*b)
```
```{r l2pex2,results ="asis",echo=FALSE}
cat("```r\n")
cat(paste0("l2p(",a,"+",b,"*11)\n"))
cat("```\n")
```
```{r l2pex2r,echo=FALSE}
l2p(a+11*b)
```
]
