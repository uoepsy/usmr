...
myvals <- seq(-5,5,by=.5)
ggplot() + geom_hline(yintercept=myvals,colour="blue",size=2) + xlim(-2,2) +
  labs(title=("intercept = {closest_state}")) +
  annotate("segment",x=0,xend=0,y=0,yend=myvals,size=1.5,colour="orange",arrow=arrow()) +
...
ggplot() + geom_abline(intercept=0,slope=myvals,colour="blue",size=2) + xlim(-2,2) +
  labs(title=("slope = {closest_state}")) +
  annotate("segment",x=0,xend=1,y=-2.5,yend=-2.5,colour="black",size=1.5) +
  annotate("segment",x=1,xend=1,y=0,yend=myvals,size=1.5,colour="orange",arrow=arrow()) +
...
$$y=\color{blue}{\textrm{intercept}} \cdot{} \color{orange}{1} + \color{blue}{\textrm{slope}} \cdot{} \color{orange}{x}$$
...
b0 <- seq(-4,4,by=1)
b1 <- seq(-2,2,by=.5)
vals <- as.data.frame(expand.grid(b0,b1))
sv <- 1:length(vals[,1])

ggplot() + xlim(-10,10) + ylim(-10,10) +
  geom_abline(intercept=vals[sv,1],slope=vals[sv,2]) +
  transition_states(sv) +
  labs(title=("intercept = vals[{closest_state},1], slope = vals[{closest_state},2]"))
```
...
$$\hat{y} = \color{blue}{b_0} + \color{blue}{b_1} \cdot{} \color{orange}{x_i}$$
