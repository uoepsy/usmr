...
#| echo: false
#| fig.asp: 0.8
t <- tibble(r=replicate(10000,randomCor(50)))
p <- t %>% ggplot(aes(x=r)) + geom_density(size=2) +
  ggtitle("10000 correlations of 50 random pairs") +
  ylim(0,3) + xlim(-.6,.6)
p
```
...
crit <- cor(dat$BloodAlc,dat$RT)

ggplot(tt,aes(x=x,y=y)) +
  # STEP 1: Draw the filled red area first
  geom_ribbon(
    data = . %>% filter(x > crit),
    aes(ymin = 0, ymax = y),
    fill = "red",
  ...
  geom_vline(aes(xintercept=crit),linetype="dotted",size=1,colour="red") +
  # STEP 2: Draw the black line on top of the fill
  labs(x = "r", y = "density") + ylim(0,3) + xlim(-.6,.6) +
  ggtitle("10000 correlations of 50 random pairs") +
  annotate("text",y=2.4,x=crit-.05,
           label=bquote(r==.(crit)),
           size=9,colour="red",hjust=1)
...
- distribution of $r$ under chance


:::: {.fragment fragment-index=1}

- probability of getting observed $r$ or higher

- estimate using $t$ distribution with $n-2$ df
...
p + stat_function(fun=dtr,colour="red",args=list(n=50),linewidth=2) +
  ylab("density")
```
...
## [Reading Week 20--26 October]{.red}
