...
p2 <- p + stat_function(fun=dtr,colour="red",args=list(n=30),linewidth=2) +
  ylab("density")
p2
```
...
#density_data <- ggplot_build(p2)$data[[2]]

tt <- ggplot_build(p2)$data[[1]]

p +
  # STEP 1: Draw the filled red area first
  geom_ribbon(
    data = tt |> filter(x > 0.4),
    aes(ymin = 0, ymax = y),
    fill = "red",
    alpha = 0.8
  ) +
  # STEP 2: Draw the black line on top of the fill
  labs(x = "r", y = "density") + xlim(.8,.8)
...
- can be mapped to $t$ distribution
