...
- `mean(profit)/se` (`r .rround(mean(profit)/se,2)`) is "number of standard errors from zero"
...
- you can take _any_ mean, and _any_ standard deviation, and produce "number of standard errors from zero"
...
- in part 2,mean profit was £`r pr`; standard error was `r se`
...
- for $t(11)=`r mean(profit)/se`$, $p=`r .pv(pt(mean(profit/se),11,lower.tail=FALSE))`$
