...
```{r simple, echo=c(2:6)}
set.seed(5234)
ourPop <- read_csv("https://uoepsy.github.io/data/surveydata_allcourse22.csv") %>%
  filter(course == "usmr", year == 2022)
nrow(ourPop)
ourDat <- ourPop %>% sample_n(20)
  # take 20 data points
```

```{r s2,include=FALSE}
lmp <- function(x) {
  m <- lm(emot_stability ~ loc,data=x)
...
ourDat %>% ggplot(aes(x=loc, y = emot_stability)) +
  geom_point(size=3)
```
]
...
```{r lm1, eval=F}
mod <- lm(emot_stability ~ loc, data=ourDat)
```
...
p1 <- ourPop %>% ggplot(aes(x=loc, y = emot_stability)) +
  xlim(c(-1,12)) + ylim(0,20) +
  geom_segment(aes(x=0,y=0,xend=0,yend=coef(mod)[1]),colour="red") +
  annotate("text",x=.3,y=coef(mod)[1]-1,label=expression(b[0]),size=5,colour="red")
p1
```
]
???
...
p2 <- p1 +
  geom_segment(aes(x=7,xend=8,y=lmf(7),yend=lmf(7)),
               arrow=arrow(ends = "both",length = unit(.03,"native"))) +
  annotate("text",x=7.5,y=lmf(6),label="1",size=5) +
  geom_segment(aes(x=8,xend=8,y=lmf(7),yend=lmf(8)),
               colour="red") +
  annotate("text",x=8.4,y=lmf(7.5),label=expression(b[1]),colour="red",size=5) +
  geom_segment(aes(x=0,xend=1,y=lmf(0),yend=lmf(0)),
                arrow=arrow(ends = "both",length = unit(.03,"native"))) +
  geom_segment(aes(x=1,xend=1,y=lmf(0),yend=lmf(1)),
                colour="red") +
  annotate("text",x=1.4,y=lmf(0),label=expression(b[1]),colour="red",size=5)
p2  
```
]
...
iS <- function(size) {
  tt <- ourPop %>% sample_n(size)
  m <- lm(emot_stability ~ loc,data=tt)
  return(coef(m))
}

cS <- as_tibble(t(replicate(100,iS(20)))) 

ourPop %>% ggplot(aes(x=loc,y=emot_stability)) +
  xlim(c(0,40)) + ylim(c(0,40))+
  geom_abline(data=cS,aes(intercept=`(Intercept)`,slope=loc),alpha=.1,colour="blue")
```
]
...
ourDat <- ourDat %>% mutate(pred=predict(m1),mES=mean(emot_stability))
p1 <- ourDat %>% ggplot(aes(x=loc,y=emot_stability,yend=pred))

pTot <- p1 + geom_abline(intercept=mean(ourDat$emot_stability),slope=0,size=1) +
  geom_segment(aes(yend=mean(mES),xend=loc),linetype="dotted",colour="red",size=1) +
  geom_point(size=3)
pTot
```
...
pRes<- p1 + geom_segment(aes(xend=loc),linetype="dotted",colour="red",size=1) +
  geom_smooth(method="lm",se=FALSE) +
  geom_point(size=3)
pRes
```
...
pMod <- p1 + geom_segment(aes(y=mES,xend=loc),colour="red",linetype="dotted",size=1) +
  geom_hline(yintercept = mean(ourDat$emot_stability),size=1) +
  geom_smooth(method="lm",se=FALSE) +
  geom_point(size=3)
pMod
```
