...
.pull-right[
![](lecture_6_files/img/playmo_brain.jpg)
]
???
- is there anything more the data about exercise and brain volume can tell us?

- so far we know they're correlated, but it would be good if we could something about _how good/bad_ exercise is
...
???
- this kind of information is based on the assumption that the relationship between activity and brain volume is _linear_
...
p2 <- p1 +
  ...
  annotate("text",x=4.5,y=lmf(3.5),label="1",size=5) +
...
p2 + geom_abline(intercept=coef(mod)[1],slope=coef(mod)[2],colour="blue",size=1)
```
]
...
p2 +
  geom_abline(intercept=coef(mod)[1],slope=coef(mod)[2],colour="blue",size=2) + 
  geom_point(size=3)
```
]
???
