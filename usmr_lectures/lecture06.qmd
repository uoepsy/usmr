...
dfR |> ggplot(aes(x=x,y=y)) +
  ...
  ggtitle(expression(paste(hat(y)," = ",3 + 1 %.% x )))
```
...
- for a normal distribution, what values _should_ (say) 5%, or 10% of the observations lie below?
...
## Q-Q Plots {.smaller visibility="uncounted"}
...
- from our residuals, what values _are_ (say) 5%, or 10%, of observations found to be less than?
...
2. we need to talk about **influence**
