...
For a 95% interval^[
For other intervals, such as a 90% interval, we need to know the point at which 5% is either side of a normal distribution (i.e., giving us the middle 90%). We can do this with `qnorm()` (we introduced this in [the reading on sampling](02b_sampling.html#normal-distributions){target="_blank"}).  
