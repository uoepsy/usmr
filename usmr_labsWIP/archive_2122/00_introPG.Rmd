...
__PLEASE DO NOT USE THE SERVER FOR YOUR DISSERTATIONS.__ 

:::


`r optbegin("Option A: The RStudio PPLS Server",olabel=FALSE)`
...
Any problems, drop us an email at [pg.ppls.stats@ed.ac.uk](mailto:pg.ppls.stats@ed.ac.uk).  
`r optend()`


`r optbegin("Option B: Installing R and RStudio locally. ",olabel=FALSE)`
:::red
...
You can also always email us at [pg.ppls.stats@ed.ac.uk](mailto:pg.ppls.stats@ed.ac.uk).   
:::
...
The bottom-right window actually does some other useful things, for instance showing you the files associated with your project. 
...
**We're going to use Rmarkdown documents for the assignment of this course, but do not worry too much right now about them - we'll get into it how to write them later on in the course**, but it basically involves writing normal text interspersed with "code-chunks" (i.e., chunks of code!).
...
In your __R script__ (not the console), type `library(cowsay)` and hit enter. This *loads* the package for us to use it.  
...
`r qbegin("Task - Install and load the Tidyverse packages",qlabel=FALSE)`
In the __console__, install the package "tidyverse". Then, in __your R script__, load the package using `library()`.  
