...
`r qbegin()`
There were lots of 3d plots in the readings, but we typically don't visualise multiple regression models in this way. One option is to visualise the model estimated relationship between the outcome and a predictor of interest in 2-dimensions (a bit like looking at a 3d plot from just one side, so the surface will look like a line). 
...
- This might seem difficult at first, but actually we can just use the __sjPlot__ package (`plot_model` function) that we used last week.  
- to visualise just one association, you might need the `terms` argument in `plot_model()`. Don't forget to look up the documentation by typing `?plot_model` in the console
