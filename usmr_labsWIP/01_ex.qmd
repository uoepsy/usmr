...
:::hints
__Hints:__ You'll need the `read.csv()` function. Remember to assign it a name to store it in your environment. Section 1B contains [an example of reading in data from a URL](01b_data.html#Basic_Data_Wrangling){target="_blank"}. You'll then want to play with functions like `dim()` and `head()`.  
:::
`r qend()`
...
:::hints
__Hints:__ You'll want to think about how we access data via asking for those entries that meet a specific _condition_ (see [this part of Section 1B](01b_data.html#Accessing_by_a_condition){target="_blank"})  
:::
`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`
```{r}
...
:::hints
__Hints:__ This is a slightly complex one. The bit on [more complex conditions in Section 1B](01b_data.html#More_complex_conditions){target="_blank"} might help you here.  
:::
`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`
```{r}
...
:::hints
__Hints__: Try looking up the help documentation for the function `round()`. Try playing with it in the console, e.g. `round(c(3.5, 4.257, 1.1111))`. You may find it helpful to look back at [Section 1B: 'Adding/Changing a variable'](01b_data.html#addingchanging-a-variable){target="_blank"}.  
:::
`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`
```{r}
