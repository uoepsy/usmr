...
:::column-margin
Our recommendation is that you have an R project for this course, and use a new script for each week of work. See [the tip about "R projects" in Reading 1A](01a_R.html#_The_Four_RStudio_Panes){target="_blank"}.  
:::
...
:::hints
__Hints:__ You'll need the `read.csv()` function. Remember to assign it a name to store it in your environment. Reading 1B contains [an example of reading in data from a URL](01b_data.html#Basic_Data_Wrangling){target="_blank"}. You'll then want to play with functions like `dim()` and `head()`.  
:::
`r qend()`
...
:::hints
__Hints:__ You'll want to think about how we access data via asking for those entries that meet a specific _condition_ (see [this section of Reading 1B](01b_data.html#Accessing_by_a_condition){target="_blank"})  
:::
`r qend()`
...
:::hints
__Hints:__ This is a slightly complex one. The section on [more complex conditions in Reading 1B](01b_data.html#More_complex_conditions){target="_blank"} might help you here.  
:::
`r qend()`
...
:::hints
__Hints__: Try looking up the help documentation for the function `round()`. Try playing with it in the console, e.g. `round(c(3.5, 4.257, 1.1111))`. You may find it helpful to look back at [Reading 1B/'Adding/Changing a variable'](01b_data.html#AddingChanging_a_variable){target="_blank"}.  
:::
`r qend()`
