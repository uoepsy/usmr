...
We have actually already been talking about various aspects of one of the most important distributions in statistics, those that we call "normal distributions". Sometimes you will hear it referred to as a "bell curve" due to its resemblance to the shape of a bell. We started to see distributions (@fig-bellsd) which were roughly bell-shaped, and saw how the shape of such distributions depends on parameters such as the mean and variance/standard deviation^[remember that standard deviation is $\sqrt{\text{variance}}$]. 
```{r}
#| label: fig-bellsd
#| fig-cap: Data From Chapter 3, scores on test 2 have a larger standard deviation, as seen in the distribution of test 2 being wider.  
#| code-fold: true
...
tibble(
  ...
  ggplot(.,aes(x=x))+
  geom_ribbon(aes(ymin=0, ymax=y, fill="100"), alpha=0.3)+
  geom_ribbon(aes(ymin=0, ymax=y2, fill="150"), alpha=0.3)+
  geom_ribbon(aes(ymin=0, ymax=y3, fill="50"), alpha=0.3)+
  theme_classic()+
  scale_y_continuous(NULL, breaks=NULL,)+
  theme(axis.line.y = element_blank(), axis.text.x = element_blank(), 
        axis.title.x = element_blank(), legend.position="bottom")+
...
#| fig-cap: "Snapshots from [lecture slides](../lectures/lecture_2.html#5) on measurement"
#| echo: false
knitr::include_graphics("images/hypothesis/measurelec.png")
```

<!-- Suppose we were measuring 2 different sticks, and we measured each one 50 times. All of the 100 measurements we took might look like they are _not_ normally distributed (@fig-sticks, Left), but once we account for the systematic differences in these observations (e.g. which stick is measured), then we see that we actually have two (roughly) normally distributed samples:  -->
<!-- ```{r} -->
<!-- #| label: fig-sticks -->
<!-- #| fig-cap: "Left: 100 measurements. Right: 100 measurements split by which stick is being measured" -->
<!-- #| echo: false -->
<!-- #| fig-height: 2 -->
<!-- set.seed(410) -->
<!-- tibble( -->
<!--   y =c(rnorm(50, .075, .004), rnorm(50, .087, .004)), -->
<!--   stick = rep(letters[1:2],e=50) -->
<!-- )->df -->
<!-- ggplot(df,aes(x=y))+geom_density()+ -->
<!--   ggplot(df,aes(x=y))+geom_density(aes(col=stick)) -->
<!-- ``` -->
...
Let's return to the IQ data we saw in the [previous chapter](03_measurement.html#histograms). We have our observed sample distribution of IQ scores: 
...
When we discussed IQ [above](#normal_distributions), we had the idea of an underlying _population distribution_ of everybody's IQ scores. When we randomly choose one person (a sample of $n=1$), we might get someone who has an IQ a bit lower, or a bit higher, than the average.  

The same applies when we take are taking the mean of a sample. Suppose we are interested in the average "life satisfaction rating" (@fig-lifesat) for the entire adult population. If we take a sample of say, $n=30$ people, we might just happen to have a few more people in our sample who are highly satisfied, or a few more people who are more dissatisfied. Consequently, the mean of our sample will be a little bit higher/lower than the population average.
```{r}
#| label: fig-lifesat
...
When we use the mean rating from our sample as an _estimate_ of the mean rating in the _population_, it would be good to be able to quantify how much certainty we have in that estimate - i.e. how much do we expect the mean life satisfaction rating from samples of $n=30$ to vary just due to our random sampling?
...
If we could, we would collect data from _everyone_ in Scotland, and might find distributions like these: 

```{r}
#| label: fig-cltpop
#| fig-cap: "Population distributions on three variables (numbers like 4e+05 are a compact way of writing 400000)"
#| echo: false
#| fig-width: 10
#| fig-height: 3
...
One thing that is often useful to do when using a sample estimate is to construct a _range of plausible values_, providing a view of the uncertainty in our estimate.   
...
However, in real life remember we _don't have_ lots of sample means. We just have the one:  
...
Combine these with the [rules of thumb](#normal_distributions) for normal distributions that we saw above, where 68%/95%/99.7% of values will lie within 1/1.96/3 standard deviation of the mean.  
