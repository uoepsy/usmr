...
- What happens when the association between two variables _depends on_ the level of a third? How can we build this into a linear model?   
...
- For people who are APOE4 negative (the reference level, see [Chapter 11 #multiple-categories-multiple-regression](11_mlr.html#multiple-categories-multiple-regression){target="_blank"}), living where the Air Quality Pollution Index is 0 (no pollution), the estimated score on the MMSE (mini mental state exam) is `r round(coef(eg2mod)[1],2)`.  
...
Then the default is to take the alphabetical ordering. We can change the ordering using functions that "relevel" a factor. Note, this only works if the variable is _already_ a `factor` in R (see [Chapter 3 #categorical](03_measurement.html#categorical){target="_blank"} for a reminder of 'factors').   
...
When we are using the default contrasts coding (treatment - see [Chapter 14 #contrasts](14_catpred.html#contrasts){target="_blank"}) in R, then our coefficients for the individual predictors represent moving between the dots in @fig-sumplot.  
