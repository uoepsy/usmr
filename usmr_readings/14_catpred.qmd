...
Since we've started working with linear regression, we've seen a few examples of categorical variables as predictors in linear models, and most (if not all) of these have been binary predictors - i.e. having just *two* categories (yes/no, dog/cat, phonics/word). We talked initially ([Chapter 10 #binary-predictors](10_slr.html#binary-predictors){target="_blank"}) about how these get entered in the model as 0s and 1s.  
...
Why? because this allows us to use the F test as a "joint test" (see [Chapter 11 #model-comparisons](11_mlr.html#model-comparisons){target="_blank"}) of whether including all the information about groups explains a significant amount of additional variance in our outcome (i.e. if their inclusion provides a significant reduction in residual sums of squares).
