...
When we introduced the linear regression model (see [Chapter 10](10_slr.html)), we talked mainly about the intercept and the slope (collectively referred to as "the coefficients"). We saw that the model summary output (from `summary(model)`) gave us some tests next to each coefficient: 
...
for(i in seq(min(x1),max(x1),length=20)){
  xx2 = expand.grid(x1=i,
           x2=seq(min(x2),max(x2),length=20))
  yx2 = predict(fit,xx2)
  lines(trans3d(xx2$x1,xx2$x2,yx2,p),col="darkorange",lwd=1)
}
for(i in seq(min(x2),max(x2),length=20)){
  xx1 = expand.grid(x2=i,
           x1=seq(min(x1),max(x1),length=20))
  yx1 = predict(fit,xx1)
  lines(trans3d(xx1$x1,xx1$x2,yx1,p),col="blue",lwd=1)
}
...
for(i in seq(min(x1),max(x1),length=20)){
  xx2 = expand.grid(x1=i,
           x2=seq(min(x2),max(x2),length=20))
  yx2 = predict(fit,xx2)
  lines(trans3d(xx2$x1,xx2$x2,yx2,p),col="darkorange",lwd=2)
}
for(i in seq(min(x2),max(x2),length=20)){
  xx1 = expand.grid(x2=i,
           x1=seq(min(x1),max(x1),length=20))
  yx1 = predict(fit,xx1)
  lines(trans3d(xx1$x1,xx1$x2,yx1,p),col="blue",lwd=2)
}
