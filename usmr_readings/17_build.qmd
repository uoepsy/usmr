...
To see how this causes a problem, suppose that there was **no** effect of `EnclosureType` on `Curious`, meaning that we shouldn't be able to see any pattern between the two (hence the lack of an arrow between the two in the figure above).
...
rohrer - causal inference cookbook  
pearl - crash course  
...
We may end up with a much more complex diagram^[ ![](images/chimpdag/nonsense.png) this is just as many things as I could think of in ~10 minutes! I don't know anything about how zoos work, or about primatology, so I don't know how many of these arrows are justified!]
...
If you feel the draw of experimental work, then this is all useful stuff, but we would strongly recommend devoting time to reading this book (it's available online for free!):  
...
If you're excited by observational research, then you've got your work cut out! However, there are lots and lots of tutorials out there, and the field is constantly developing. One particular book that stands out (for me at least) for its engaging and accessible language is listed below (it's available online for free!):

Huntington-Klein, N. (2025). *The effect: An introduction to research design and causality*. CRC Press. <https://theeffectbook.net/>
...
Even successfully maintaining that "I'm only describing things" does not absolve us from _thinking_ about these things.  
...
In practice, we rarely have the full population data, and so we take a random sample and conduct a survey. However, we need to be careful to think about the things that might lead people to decide to complete (or not complete) a questionnaire on depression. 
Typically, people who are severely depressed a wya less likely to ever fill out these surveys.
So we can think of our population as containing people like this: 
```{r}
head(dep_fullpop)
```

What that means is, when we take our sample, we are far less likely to see those people, and we are therefore likely to _underestimate_ the prevalence of depression: 
...
And that's because the problem is that our statistics misrepresent the data generating process. 
...
All we ever report are statistical associations, but the question is really *which* association do we want?  


the term now appears to be sometimes used to maintain plausible deniability. 
