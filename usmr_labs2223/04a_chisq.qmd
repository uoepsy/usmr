...
:::qq
__Q1:__ What is your intuition about the distribution of all students' birth-months?  
...
:::
:::qq
__Q2:__ We're going to perform a statistical test to assess the extent to which our data conforms to the hypothesis that people are no more likely to be born on one month than another.  
...
:::
:::qq
__Q3:__ Given that there are there are `r nrow(survey_data)` people who entered their birth-month, how many observations would we *expect* to find with a birthday in January? And in February? ... and so on? 
:::
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
:::qq
__Q4:__ The code below creates counts for each month: 
```{r}
#| eval: false
survey_data %>%
    group_by(birthmonth) %>%
...
:::
:::qq
__Q5:__ You can see the distribution of $\chi^2$ distributions with different degrees of freedom below.  
```{r}
#| label: fig-chidist
#| echo: false
...
:::
:::qq
__Q6:__ Which months of year had the highest contributions to the chi-square test statistic?  
:::
:::column-margin
...
:::
:::qq
__Q7:__ According to the internet (that reliable source of information!), 76% of people in the world have brown eyes, 10% have blue, 5% hazel, 5% amber, 2% green, 1% grey, and 1% have some other eye colouring (red/violet/heterochromia).
...
:::
:::qq
__Q8:__ What are the *observed* proportions of our sample with each eyecolour?  
...
:::qq
__Q9:__ Produce a plot and a table to display the relationship between whether or not the customer left a tip, and what (if any) card they received alongside the bill.  
:::
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r}
...
:::
:::qq
__Q10:__ What would you *expect* the cell counts to look like if there were no relationship between what the waiter left and whether or not the customer tipped?  
:::
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
:::
:::qq
__Q11:__ Just like we gave the `chisq.test()` function a table of observed frequencies when we conducted a goodness of fit test in earlier exercises, we can give it a two-way table of observed frequencies to conduct a test of independence.  
