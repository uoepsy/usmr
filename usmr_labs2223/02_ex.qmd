...
:::qq
__Q1:__ Create a new variable in the dataset which indicates whether people are taller than 6 foot (182cm).
:::
:::column-margin
...
:::qq
__Q2:__ What percentage of respondents to the survey are greater than 6 foot tall?
:::
:::column-margin
Try `table()`, and then think about how we can convert the counts to percentages (what does `sum()` of the table give you?).  
See also the [discussion of categorical data](02a_measurement.html#categorical){target="_blank"}.
:::
:::bs75
:::

:::qq
__Q3:__ how many of USMR students in `r paste0(as.numeric(substr(Sys.Date(),1,4)),"/",as.numeric(substr(Sys.Date(),3,4))+1)` are born in the same month as you? 
:::
:::column-margin
...
:::qq
__Q4:__ Calculate the mean and standard deviation of heights of all respondents to the survey.  
...
We're likely to have missing data in here, so `na.rm=TRUE` will be handy (see the reading on [numeric variables](02a_measurement.html#numeric){target="_blank"})
:::
:::bs50
:::

:::qq
__Q5:__ Plot the distribution of heights of all respondents. 
Try to make it 'publication ready'.  
:::
:::column-margin
No `hist()` here, we're going to want [ggplot!](02a_measurement.html#ggplot){target="_blank"}.  
:::

:::qq
__Q6:__ For respondents _from each of the different courses_, calculate the mean and standard deviation of heights.  
:::
:::column-margin
...
:::qq
__Q7:__ Plot the distributions of heights for each course.  
...
:::qq
__Q8:__ What proportion of respondents to the survey are taller than you?  
:::
:::column-margin
...
:::qq
__Q9:__ Suppose we are interested in the average life-satisfaction rating of MSc students at Edinburgh University.  
...
:::qq
__Q10:__ For the USMR students in `r paste0(as.numeric(substr(Sys.Date(),1,4)),"/",as.numeric(substr(Sys.Date(),3,4))+1)`, calculate the following: 
...
:::
:::column-margin
You can do this with things like `mean(data$variable)`, or you can do it all in tidyverse (see the [example of `summarise` in the intro to tidyverse](02a_measurement.html#tidyverse-and-pipes){target="_blank"}). If you do this all inside `summarise()` then you might find the function `n()` useful.
:::

:::qq
__Q11:__ Using your answers to the previous question, construct a 95% confidence interval for the mean life satisfaction rating.  
...
:::
:::column-margin
The previous question gives you all the pieces that you need. You'll just need to put them together in the way seen in [the section on CIs](02b_sampling.html#confidence-intervals){target="_blank"}.  
