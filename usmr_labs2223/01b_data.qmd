...
To store a sequence of numbers into R, we _combine_ the values using `c()` and give the sequence a name. A sequence of elements all of the same type is called a **vector**.
...
In this particular case, it would make everything a __character__ (sequence of letters), check the "", as it would be harder to transform a word into a number!
...
While we can manually create data like we did above, more often we will need to _import_ into R some data which has been created elsewhere (like in excel, or by some software which is used to present participants with experiments).  

Let's suppose we have instead entered data in something like Microsoft Excel, or LibreOffice Calc, or whatever spreadsheet software we have available (Figure @fig-excel).  
```{r}
#| label: fig-excel
#| echo: false
...
<!-- (note: if you are using RStudio on a server - i.e. accessing it via a web-browser - then you will need to upload the file you just saved to the server).   -->
...
TODO redo video with script not rmd
<center><video width="320" height="240" controls>
...
To get more insight into what the data actually are, you can either use the structure `str()` function:
```{r}
str(beatles)
```
...
We will be using these packages quite a lot in this course, so you might want to install them now:  
...
Hopefully, if you are following along with us, you are writing your code in a script, and not in the console. __Remember: Code that you write in the console is *not* saved__.  

Sometimes, however, for certain things, we might not want to keep a record of that bit of code. 
...
- If you clear your environment, restart R, and run your script all at once (see [A first look at R & RStudio/Good Habits](01a_R.html#Good_Habits)), it shouldn't produce errors and should get you back to your current place  
...
Now that we've started to learn about vectors and dataframes, we're going to look at reading in some data, exploring some of the variables, and the syntax we can use to access and edit certain bits of a dataset.    
...
Thinking back to the previous reading [when we first introduced R](01a_R.html#R_is_a_calculator), we talked briefly about "logical operators". Specifically, the operators `&`, `|`, and `!` (for "and", "or"," and "not"), will come in handy now.  
...
We can change these by __assigning them a new value__ (remember the `<-` symbol). In doing so, we _replace_ / _overwrite_ / _reassign_ the entry in the 2nd row and 6th column of the data (`starwars2[2,6]`) with the value "Droid". 
```{r}
