...
:::frame
We're going to play with some data on a sample of licensed pets from the city of Seattle, USA.  
It can be downloaded (or read directly into R) from [https://uoepsy.github.io/data/pets_seattle.csv](https://uoepsy.github.io/data/pets_seattle.csv).  
It contains information on the license ID, year of issue, as well as the species, breeds and weights of each pet. You can find a data dictionary in @tbl-petdict
```{r}
#| label: tbl-petdict
...
:::hints
__Hints:__ You'll need the `read.csv()` function. Remember to assign it a name to store it in your environment.  
[1B #basic-data-wrangling](01b_data.html#Basic_Data_Wrangling){target="_blank"} contains an example of reading in data from a URL. 
...
`r qbegin(3)`
Subset the data to only the data for the dogs, and store this object as another named object in your environment.   
Subset the data to only the data for the cats, and store this object as another named object in your environment.   
...
:::hints
__Hints:__ You could do this using the original data you read in from question 1, or use the subsets you created in question 3. You'll again want to supply a condition within square brackets `data[?==?]`. That condition may well have something to do with being equal to the `min()` or the `max()` of some variable.  
:::
`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`
