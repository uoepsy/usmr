...
p1 <- ggplot(monkeytoys, aes(x=obj_type, y=exploration_time,
  ...
  labs(subtitle="exploration_time~obj_type")+
...
p2 <- broom::augment(model2, interval="confidence") |>
  ...
  scale_y_continuous(breaks=seq(0,22,2))+
  labs(subtitle="exploration_time~age+obj_type")
...
But our scores are based on the sum of 9 questions that can each score from 1 to 5. So surely that means the minimum someone could score would be 5, and the maximum they could score would be 45?  
...
Because we had that very happy person (`happy` variable was 47) that we replaced with an `NA`, we have one fewer bit of data to fit our model to.  
...
```
...
F-statistic: 98.09 on 3 and 72 DF,  p-value: < 2.2e-16
```
