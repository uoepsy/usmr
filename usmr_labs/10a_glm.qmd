...
The GLM involves a little bit of trickery, in which we translate something that is inherently not linear (e.g. the _probability_ of Y, or the _rate_ of Y) into something which is both linear and unbounded, and model that thing instead.  
...
:::int
For every additional month older a child is, the odds of them taking the marshmallow decreased by `r round(exp(coef(mallowmod)[2]),2)` ($z = `r round(coefficients(summary(mallowmod))[2,3],2)`$, $p < .001$).  
:::
...
:::int
For every additional month older a child is, the odds of them taking the marshmallow decreased by `r round(exp(coef(mallowmod)[2]),2)` (95% CI [`r paste0(round(exp(confint(mallowmod)[2,]),2),collapse=", ")`]).  
:::
