...
dag
emot_stability confound

how does loc influence sleeprating _after_ accounting for emot_stability?  
...
usmrdata <- 
  ...
  filter(!(pseudonym %in% "Definitely not Ian"))
set.seed(993)
...
m <- lm(sleeprating ~ emot_stability + loc, usmrdata)
...
is birth month associated with conscientiousness?  

hint 
note the question is more a matter of "is"/"does", and not "what is"/"how does". 
to answer this question, we might be more inclined to conduct a 

`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`

```{r}
m <- lm(conscientiousness ~ birthmonth, usmrdata)
anova(m)
```
