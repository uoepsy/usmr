...
The standard error for a correlation $r$ is quantifiable as $\sqrt{\frac{(1-r^2)}{(n-2)}}$. We can think of this as what variance gets left-over ($1-r2$) in relation to how much data is free to vary ($n-2$ because we have calculated 2 means in the process of getting $r$).  
This logic maps to how our standard error of the mean was calculated $\frac{\sigma}{\sqrt{n}}$ - it is $\frac{\text{leftover variation}}{\text{free datapoints}}$.  
