...
`qnorm`!  (see the end of [Chapter 5 #uncertainty-due-to-sampling](https://uoepsy.github.io/usmr/2627/readings/05_inference.html#uncertainty-due-to-sampling){target="_blank"})
...
`r qbegin(qcounter())`
As we learned in [Chapter 6 #t-distributions](https://uoepsy.github.io/usmr/2627/readings/06_inference2.html#t---distributions){target="_blank"}, the sampling distribution of a statistic has heavier tails the smaller the size of the sample it is derived from. In practice, we are better using $t$-distributions to construct confidence intervals and perform statistical tests.  
...
- you can see the step-by-step calculation of a one sample t-test in [Chapter 6 #one-sample-t-test](https://uoepsy.github.io/usmr/2627/readings/06_inference2.html#one-sample-t-test){target="_blank"}. 
...
::: {.callout-tip collapse="true"}
#### Hints  
There are some quick example write-ups for each test in [Chapter 6 #basic-tests](https://uoepsy.github.io/usmr/2627/readings/06_inference2.html#basic-tests){target="_blank"}
:::
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r include=FALSE}
...
```{r}
surveydata <- 
  read_csv("https://uoepsy.github.io/data/usmr26survey_historical.csv")
```
:::
...
For a sense of what looks "normal", try running this code a few times over. Each time it runs, it takes a random sample of 200 observations, and then plots a histogram and a QQplot. By doing it lots of times, you can get a sense of how much these plots tend to vary even when the data are *definitely* drawn from a normal distribution. 

```{webr}
# take a sample
x <- rnorm(200, mean = 100, sd = 15)
# basic plots side by side:
par(mfrow=c(1,2))
hist(x)
qqnorm(x);qqline(x)
```
...
The data are available at [https://uoepsy.github.io/data/gerritysim.csv](https://uoepsy.github.io/data/gerritysim.csv)  
...
We can again use the data from the past surveys: [https://uoepsy.github.io/data/usmr26survey_historical.csv](https://uoepsy.github.io/data/usmr26survey_historical.csv)  
