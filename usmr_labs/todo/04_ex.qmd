...
> **Research Question:** Is the probability that a USMR student in 2026 prefers Dogs over Cats greater than 50%?

:::frame
__Data: USMR 2026! It's YOU!!__  

We've used the survey data from previous years for a few weeks now, but let's start using the data that *you* provided!  

You can find the results from the `r substr(Sys.Date(),1,4)` survey at [https://uoepsy.github.io/data/usmr2026.csv](https://uoepsy.github.io/data/usmr2026.csv)  

```{r}
usmr26 <- 
  read_csv("https://uoepsy.github.io/data/usmr2026.csv")
```
:::
...
If the probability was 50%, or 0.5, then we would expect the numbers to be equal. We have `r sum(!is.na(usmr26$catdog))` responses here, so we would expect `r sum(!is.na(usmr26$catdog))/2` to be team cats, and `r sum(!is.na(usmr26$catdog))/2` to be team dogs!  
...
- You can see an example of this test in [Chapter 7 #binomials](https://uoepsy.github.io/usmr/2627/readings/07_chisq.html#binomials){target="_blank"}.   
...
#| echo: false
table(usmr26$catdog) |>
  binom.test(p = 0.5, alternative = "less")
```
...
> **Research Question:** Are USMR 2026 students more likely to be born in certain months than others?  
...
```{r}
barplot(table(usmr26$birthmonth))
```

A ggplot option:
```{r}
ggplot(data = usmr26, aes(x = birthmonth)) +
    geom_bar() +
    labs(x = "- Birth Month -")
```
`r solend()`
...
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
There are `r sum(!is.na(usmr26$birthmonth))` people who have non-NA values (`sum(!is.na(usmr26$birthmonth))`).  

Under the null hypothesis, we would expect $\frac{1}{12} \times$ `r sum(!is.na(usmr26$birthmonth))` = `r round(sum(!is.na(usmr26$birthmonth))/12,2)` observations born in each month. 
`r solend()`
...
```
(A shortcut for this would be `usmr26 |> filter(!is.na(birthmonth)) |> count(birthmonth)`)  
...
::: {.callout-tip collapse="true"}
#### Hints  
This was all done in the step-by-step example of a $\chi^2$ test in [Chapter 7 #chi2-goodness-of-fit-test](https://uoepsy.github.io/usmr/2627/readings/07_chisq.html#chi2-goodness-of-fit-test){target="_blank"}
:::
...
chi_table <- 
    ...
    summarise(
        observed = n(),
        expected = sum(!is.na(usmr26$birthmonth))/12,
...
2. Check that these results match with those provided by R's built-in function: `chisq.test(table(usmr26$birthmonth))` (the `table` function will ignore NAs by default, so we don't need to do anything extra for this).    
...
```{r}
chisq.test(table(usmr26$birthmonth))
```
`r solend()`
...
```{r}
ggplot(chi_table, aes(x = birthmonth, y = observed/nrow(usmr26))) +
...
```{r}
chisq.test(table(usmr26$birthmonth))$residuals
```

```{r}
#| echo: false
presids <- chisq.test(table(usmr26$birthmonth))$residuals
...
We saw the use of `chisq.test()` in the example goodness of fit test, [Chapter 7 #chi2-goodness-of-fit-test](https://uoepsy.github.io/usmr/2627/readings/07_chisq.html#chi2-goodness-of-fit-test){target="_blank"}  
:::
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
The data are available at [https://uoepsy.github.io/data/TipJoke.csv](https://uoepsy.github.io/data/TipJoke.csv)  
...
::: {.callout-tip collapse="true"}
#### Hints  
`plot(table(...))` will give you something. You can see one in the example $\chi^2$ test of independence,[Chapter 7 #chi2-test-of-independence](https://uoepsy.github.io/usmr/2627/readings/07_chisq.html#chi2-test-of-independence){target="_blank"}.  
:::
...
Using the data from the USMR 2026 survey: 

[https://uoepsy.github.io/data/usmr2026.csv](https://uoepsy.github.io/data/usmr2026.csv)  
