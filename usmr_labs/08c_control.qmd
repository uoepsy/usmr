...
fs <- function(){
  Age = rnorm(100, 100, 30)
  Alc = 60 + 1*Age + rnorm(100,0,3)
  Sleep = -.5*Age + -2*Alc + rnorm(100, 0, 5)
  return(c(
    bXY.Z = coef(lm(Alc~Age+Sleep))[2],
    bXY = coef(lm(Alc ~ Age))[2]
  )
  )
}
...
library(ggplot2)
ggplot(res)+
  geom_density(aes(x=bXY.Z.Age),col="red")+
  geom_density(aes(x=bXY.Age),col="darkolivegreen")
