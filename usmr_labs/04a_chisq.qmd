...
timehands <- expand_grid(
  ...
  mutate(
    n = ceiling(c(c(.8, .2)*.1, c(.5,.5)*.84, c(.2,.8)*.03) * 1000)
  ) |> uncount(n) |>
...
Really, these proportions are "the number of $x$'s out of $n$ things", and we often see this written as "$k$ successes in $n$ trials". The reason for this is because it is built upon counting up a series of independent binary observations.  

For example, if we think of what probability distribution governs a single coin flip, it would look something like the Left hand plot in @fig-bernbin. But if we are instead considering what governs the behaviour of "number of tails in 5 coin flips" then our probabilities of each possible outcome look like the Right hand panel.   

```{r}
#| echo: false
#| label: fig-bernbin
#| fig-cap: "Left: the probability distribution for a single observation (e.g. a single coin flip) with a binary outcome, where the probability of 'success' (i.e. Tails) is 0.5. Right: the probability distribution of k success in n trials, each with a binary outcome (e.g., number of tails in 5 coin flips)."
library(patchwork)
p1 <- 
...
So in our example of "the proportion of people who are left-handed", the internet (and everything on there is true, of course) tells me that 12% of the population is left-handed.  
We have 1000 people in our dataset, 
So our expectation is therefore: 



```{r}
table(timehands$isLeft) |> prop.table()
```
...
With 1 variable, we have a 1 dimensional table. For example, how many people in the data are left-handed, right-handed, or ambidextrous:  
