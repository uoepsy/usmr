...
`r qbegin("A2")`
Simulate 100 rolls of the dice, and store the results in an object in your environment. Give it an easily identifiable name.   
...
Roll the weighted die 100 times and plot the frequency distribution. Do the same for 1,000 and 10,000 rolls.  
...
`r qbegin("A5")`
Remember, `wdice()` and `dice()` are really just relying on a different function, `sample()`. Try playing around with `sample()` in the console again - what does the `prob = c(....)` bit do?  
<br>
...
When you use the assignment operator `<-` we assign a name to a value and store in R's memory.  
...
table(droplevels(pwords$type[pwords$type == "cool-macho" | pwords$type == "simple-alphanumeric"]),
      pwords$strength[pwords$type == "cool-macho" | pwords$type == "simple-alphanumeric"])
```
`r solend()`
