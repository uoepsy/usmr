...
ggplot(., aes(x=response, y=prob))+
  ...
  theme(text = element_text(size = 24))
```
The focus of this week is about categorical data - i.e. data which has a discrete set of response options. 
...
ggplot(., aes(x=response, y=prob))+
  ...
  theme(text = element_text(size = 24))
```
We will see more about this sort of stuff next week.
:::
