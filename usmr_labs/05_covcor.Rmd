...
# Now using rowSums()
sleepdtf$dtf = rowSums(sleepdtf[, 2:11])
```
`r solend()`
...
In contrasts, the relationship between height and handspan shows deviations from an "average pattern". Hence, we need to create a model that allows for deviations from the linear relationship. This is called a _statistical_ model.  
