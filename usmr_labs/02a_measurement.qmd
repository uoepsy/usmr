...
When we collect observations on a variable, the resulting set of measurements is known as a __sample distribution__. This reading walks through the various different types of data we might encounter, and some of the metrics we use to summarise distributions of different types.  

We will use a dataset on some of the most popular internet passwords, their strength, and how long it took for an algorithm to crack it. The data are available online at [https://uoepsy.github.io/data/passworddata.csv](https://uoepsy.github.io/data/passworddata.csv).  
...
`r optbegin("be careful with conversions", olabel=FALSE)`
...
`r optbegin("Optional - Looking ahead", olabel=FALSE)`
