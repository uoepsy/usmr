...
This is the same logic as the air-pollution & APOE-4 example in [Chapter 15 #it-depends](https://uoepsy.github.io/usmr/2425/readings/15_interactions.html#it-depends){target="_blank"}.  
...
Our interaction involves a continuous variable (`hrs_sleep`) and a binary variable (`alc`). An interpretation of a similar example is in [Chapter 15 #interpretation](https://uoepsy.github.io/usmr/2425/readings/15_interactions.html#interpretation){target="_blank"}.  
...
If you get stuck, a very similar example is in [Chapter 15 #visualisation](https://uoepsy.github.io/usmr/2425/readings/15_interactions.html#visualisation){target="_blank"}.  
...
See [Ch 15 #mean-centering](https://uoepsy.github.io/usmr/2425/readings/15_interactions.html#mean-centering){target="_blank"} for an example of mean-centering a predictor in the interaction. Remember that there are multiple ways to do this - you could make a new variable first, or you could do it all _inside_ the model.  
...
```{r}
#| echo: false
df <- ctmtoys <- read_csv("https://uoepsy.github.io/data/ctmtoys.csv")
```
...
So far, we have analysed the data for two studies (not real!) of the inquisitive nature of monkeys. Initially ([week 5 exercises](05_ex.html#monkey-exploration){target="_blank"}), Archer, Winther & Gandolfi (2024) investigated age differences in exploration of novel objects, and found that older monkeys spend on average less time playing with a novel object than their younger counterparts (we looked at this with both with the linear effect of age in years, and by comparing adults to juveniles). Following this, in our [week 7 exercises](07_ex.html#more-monkeys){target="_blank"} Kennis, Lee, Zhou, Smith, Horner & Zhao (2025) wanted to see if monkeys showed a preference for different _types_ of object (i.e. ones with moving parts vs ones that are soft). They found that, after accounting for differences due to age, monkeys showed a significant preference for toys with moving parts in comparison to soft toys.  
...
Think about how the question is worded - there's no "how"/"what" etc, it's just "*are* there differences?" (this is just the same as we did last week, and in [Chapter 14 #testing-group-differences](https://uoepsy.github.io/usmr/2425/readings/14_catpred.html#testing-group-differences){target="_blank"} - try a model comparison?).  
