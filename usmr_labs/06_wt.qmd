...
```
  **NOTE** the solution has `x = ., y = eyedata` to make it clear that we are 'piping in' (using `|>`) the thing coming out of the previous lines of code, and putting it where the `_` is. `.... |> full_join(eyedata)` would do the same.  
...
This is sub-optimal (and we'll see why next semester!), but we can calculate the average `distractor_fix` for each level of `nr_blinks`, and then use _those_ in our model.  
