...
One of these was a set of items that measure a persons' "locus of control" (belief that you have control over your life, as opposed to it being decided by external forces such as fate/luck/destiny/god/the flying spaghetti monstor). We also asked you to rate your average sleep quality from 0 to 100.  
...
g1 <- dagitty( "dag {
    Locus_of_Control -> Sleep_Quality
}")
plot(graphLayout(g1))
```


Fit a linear regression model to test this association. 
...
4. Create a plot and/or table (see [7A#example](07a_slr.html#example){target="_blank"}).  
5. Write it up! 
