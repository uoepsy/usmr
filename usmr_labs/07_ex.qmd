...
`r qbegin(qcounter())`
Fit a model that estimates association between exploration time and type of object while controlling for age.  

::: {.callout-tip collapse="true"}
#### Hints

- When we add multiple predictors in to `lm()`, it can sometimes matter what order we put them in (e.g. if we want to use `anova(model)` to do a quick series of incremental model comparisons as in [Chapter 11 #shortcuts-for-model-comparisons](https://uoepsy.github.io/usmr/2526/readings/11_mlr.html#shortcuts-for-model-comparisons){target="_blank"}). Good practice is to put the thing you're interested in (the 'focal predictor') at the end, e.g.: `lm(outcome ~ covariates + predictor-of-interest)`    
...
- An example of this whole process is in [Chapter 11 #model-visualisations](https://uoepsy.github.io/usmr/2526/readings/11_mlr.html#model-visualisations){target="_blank"}.  
...
- Don't worry about interpreting the coefficients right now (we'll talk more about categorical predictors next week), but we can still test whether the inclusion of size and colour improve our model! (see [Chapter 11 #model-comparisons](https://uoepsy.github.io/usmr/2526/readings/11_mlr.html#model-comparisons){target="_blank"}).  
...
So maybe some combination of `as.numeric()` and `gsub()` might work? (see [Break week walkthrough #dealing-with-character-strings](https://uoepsy.github.io/usmr/2526/readings/zz_wt.html#dealing-with-character-strings){target="_blank"}) 
...
- If there are any observations you think have impossible values, then you could set them as NA for now (see [Break week Walkthrough #impossible-values ](https://uoepsy.github.io/usmr/2526/readings/zz_wt.html#impossible-values){target="_blank"}).  
...
[Chapter 12 Assumptions & Diagnostics](https://uoepsy.github.io/usmr/2526/readings/12_assumptdiag.html){target="_blank"} shows how we can do this. We can rely on tests if we like, or we can do it visually through plots. Getting a good sense of "what looks weird" in these assumption plots is something that comes with time.  

One way to think about residuals is that they _should_ (if our model is specified correctly) look more or less like 'randomness'.  

In order to get a sense of what 'randomness' looks like, hit the "run code" button below lots and lots of times and look at how the plots change. 
The code 

```{webr}
#| caption: "simulate data, fit model, plot assumption plots"
# simulate some data: 
x <- rnorm(100)
y <- 1*x + rnorm(100)
somedata <- data.frame(x,y)
# fit a model:
somemodel <- lm(y~x, somedata)
# plot assumption plots:
par(mfrow=c(2,2))
plot(somemodel)
```
