...
:::hints
__Hints:__ you don't need to *do* anything for this, you can find all the necessary information in `summary()` of your model.  
...
:::hints
__Hints:__ see [7A #the-error](07a_slr.html#the-error){target="_blank"}.
:::
...
4. Check for influential people. It's possible that one [really weird person](https://ianhajnosz.com/wp-content/uploads/2018/08/IMGP4662-3-724x1024.jpg){target="_blank"} could be skewing all our results.  
  - If this is the case, it might be worth conducting a 'sensitivity analysis'. Fit the model without this person (or people) - do your conclusions change?  
5. Create a plot and/or table (see [7A#example](07a_slr.html#example){target="_blank"}).  
...
```{r}
usmrdata <- 
  read_csv("../../data/surveydata_allcourse22.csv") %>%
  filter(course == "usmr", year == 2022)
```
...
usmrdata<-bind_rows(usmrdata,new)
...
From the bottom right plot above, we can see that the 65th observation is looking a bit influential..  
I wonder who that is..  
```{r}
usmrdata$pseudonym[65]
```

Ah, it's Ian! He did tell me that he didn't really pay any attention when he was filling this out. I think it might be wise to remove him.  
...
We now have a decision to make. Do we continue with Ian removed, or do we keep him in? There's not really a right answer here, but because of what we know about Ian not taking the survey seriously(!!!), I think it makes sense that we would exclude him right from the outset.  
