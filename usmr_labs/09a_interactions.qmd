...
As it happens, we can use `plot_model` to show us the how $y$ changes with $x_1$ _for some specific values of $x_2$_. Below, the red line is the association between $y$ and $x_1$ when $x_2 = -1$, the blue is when $x_2 = 0$, and the green is when $x_2 = -1$. As we can see, the slope of $y$ on $x_1$ is the same.  
...
- Person A = 50 years old with a BMI of 22  
- Person B = 50 years old with a BMI of 23
- Person C = 60 years old with a BMI of 23

The coefficient for BMI represents the difference in cognitive scores we would expect between Person A and Person B.  
Think about why this is. For a random person $i$, their model predicted score is:  
$\hat{score_i} = b_0 + b_1(age_i) + b2(BMI_i)$. 
