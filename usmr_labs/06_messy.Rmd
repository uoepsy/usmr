...
- Some functions like `read_excel()` don't allow you to download directly from a url, like we have been doing in the previous weeks.  
  - Solution 1:
...
Some of the tools we learned in [Week 1](01_categorical.html#Editing_parts_of_the_data) will come in handy here. 
...
`r qbegin("A8 (OPTIONAL DIFFICULT)")`
...
`r qbegin("A9 (OPTIONAL DIFFICULT)")`
If you attempted question **A8** above, can you edit your `outliers()` function to make it work with vectors which include `NA`s?  
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r}
outliers <- function(obs, x = 3){
...
`r qbegin("B2")`
We've seen briefly in [Week 5](05_covcor.html#Exercises:_Functions_and_Models), as well as in the lecture, the form of a linear regression model. Below, we have stated the form of the model we will use to answer our research question. We'll cover what the symbols mean in more detail next week. 
...
2. Take a look around the output of `summary(blinks_mdl)`. There's a lot of it, and we're going to go through what all the different bits mean in the coming weeks!  
