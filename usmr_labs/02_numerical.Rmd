...
`r qbegin("A2")`  
For now, let's imagine that the 10,000 entries we created in `simdata` are the entire population of interested.  
...
`r optbegin("helpful hint", olabel=FALSE)`
```{r}
# some numbers:
x1 <- c(3,1,4,2,7)
...
Look up the function `replicate()`. We can use it to do something in R lots of times!  
  ```{r}
  # run the code "1+1" 100 times: 
  replicate(100, 1+1)
  ```

Calculate the means of 1000 samples of size $n=20$ drawn randomly from the heights in `simdata`.  
...
`r qbegin("C2")`  
Using `replicate()`, calculate the means of 1000 hypothetical samples of $n = 131$.  
