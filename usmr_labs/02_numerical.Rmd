...
remember - 
we use a sample statistics to estimate a population parameter.
e.g. we use the mean difference in RT times between matching and mismatching conditions in our sample of 131 participants, to estimate the true mean difference in the population!  


Let's assume that in the population, the Stroop task doesn't do anything - i.e., in the population there is *no* difference in reaction times between the matching and mismatching conditions.  

We can simulate an hypothetical sample of the "stroop_effect" variable, of the same $n$ as our experiment.  
...
# generated sample with mean 0
rnorm(n = 131, mean = 0, sd = 5.015774) 
```

wrap the whole thing with `mean()` to find the mean of a randomly generated hypothetical sample. 
```{r}
mean(rnorm(n = 131, mean = 0, sd = 5.015774))
```

- run the code again, and then again. notice that they change a bit. why?
A: random sampling! even with mean = 0, a sample statistic will vary.  

- look up the function `replicate()`. we can use it to do something lots of times! 

- calculate the means of 1000 hypothetical samples of $n = 131$. 
- store it an object and make a histogram of it. 
```{r}
...
A: shows how, if the true population mean was 0, the mean of samples of size 131 would vary due to randomness of sampling  

- where, on our histogram, does the *observed* sample mean lie?
```{r}
mean(stroopdata$stroop_effect)
```

- if the true mean is 0, are we likely to see the mean of a sample of 131 which is `r round(mean(stroopdata$stroop_effect),2)`? 
A: no!
