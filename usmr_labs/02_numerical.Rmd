...
introduce rnorm, hist, mutate

```{r}
rnorm(10, mean = 178, sd = 10)
hist(rnorm(10, mean = 178, sd = 10))
...
`r optbegin("Optional - Help reading mathematical formulae", olabel=FALSE)`
...
`r optbegin("Optional: Why n minus 1?", olabel=FALSE)`
...
The default binwidth of the histogram here might lead you astray in guessing the mean value - the highest bar by some distance is at around 5 seconds.  
...
In both histograms, you can see that there is quite a lot of data between the values of 7 and 12.  
...
In the **tidyverse** way of doing things, there are two functions which provide similar functionality - `filter()` and `select()`. They are part of the same package as `mutate()` and `summarise()` and are often used in conjunction with the `%>%` operator to tell the functions where to find the variables (meaning we don't need to use `data$variable`, we can just use the variable name)
...
Statistical inference is the use of a sample statistic to estimate a population parameter. For example, we can use the mean difference in RT times between matching and mismatching conditions in our sample of 131 participants as an estimate of the true mean difference in the population!  

The thing is, *samples vary*. Take 10 randomly selected people and calculate the average height. Take another 10, and calculate their average height. The average from the second sample will be different from the average of the first. If you take lots of samples, the average heights will all be different, but by how much will they vary? We need to try to quantify how much sample statistics vary due to random sampling.  
  
Let's assume that in the population, the Stroop task doesn't do anything - i.e., in the population there is *no* difference in reaction times between the matching and mismatching conditions. Under this assumption, we can simulate an hypothetical sample of the "stroop_effect" variable, of the same $n$ as our experiment.  
Note that we assume the standard deviation of this sample to be the same as the sample we collected, but the mean we assume to be zero (indicating no difference between conditions).  
```{r}
# observed standard deviation
sd(stroopdata$stroop_effect)

# generated sample of 131 with mean 0
...
Calculate the means of 1000 hypothetical samples of $n = 131$.  
Store the sample means as a named object and make a quick histogram of it using `hist()`.  
...
We can then ask ourselves: "if the true population mean is 0, how likely is it that we would get a sample mean of `r round(mean(stroopdata$stroop_effect),2)` from a sample of 131?"  
