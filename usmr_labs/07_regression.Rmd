...
$$
The predicted values for the outcome are equal to our intercept, $\hat \beta_0$, plus our slope $\hat \beta_1$ multiplied by the value on our explanatory variable $x$.  
The intercept is a _constant_. That is, we could write it as multiplied by 1:
$$
\hat y = \color{blue}{\hat \beta_0}(\color{green}{1})\color{blue}{ + \hat \beta_1 }(\color{green}{x})
$$

When we specify the linear model in R, we include after the tilde sign `~` all the things which appear to the right of each of the $\hat \beta$s (the bits in green in the equartion above). That's why the 1 is included. It is just saying "we want the intercept, $\beta_0$, to be estimated".   
...
:::statbox
The parameter estimates from our simple linear regression model take the form of a line, representing the systematic part of our model $\beta_0 + \beta_1 \ x$, which in our case is $11.32 + 2.65 \ Education$. Deviations from the line are determined by the random error component $\hat \epsilon$, or "residuals" (the red lines in Figure \@ref(fig:figslr) below).  
