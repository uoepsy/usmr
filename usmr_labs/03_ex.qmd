...
`r qbegin(7)`
Now using the `t.test()` function, conduct the same test. Check that the numbers match with your manual calculations in the previous questions.  
...
Conduct a one sample $t$-test to evaluate whether the average height of students taking the USMR courses in Psychology at Edinburgh University in 2022 is different from 165cm.  
...
```
We can also take a quick look at the QQplot: 
```{r}
