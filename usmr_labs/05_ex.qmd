...
`r solbegin(label="Solution Part 1 - Reversing Items", slabel=FALSE,show=TRUE, toggle=params$TOGGLE)`
```{r message=FALSE}
...
`r solend()`
`r solbegin(label="Solution Part 2 - Creating a scale score", slabel=FALSE,show=TRUE, toggle=params$TOGGLE)`
...
::: {.callout-tip collapse="true"}
#### Hints  
You can do this all with one function, see [Chapter 9 #correlation-test](https://uoepsy.github.io/usmr/2526/readings/09_covcor.html#correlation-tests){target="_blank"}.  
:::
...
`r qbegin(paste0(qcounter()," (open-ended)"))`
...
To comment numerically on the strength of the linear association we might compute the correlation coefficient that we were introduced to in [Chapter 9: Covariance & Correlation](https://uoepsy.github.io/usmr/2526/readings/09_covcor.html)
```{r}
monkeyexp |>
...
::: {.callout-tip collapse="true"}
#### Hints
You can see how to fit linear models in R using `lm()` in [Chapter 10 #fitting-linear-models-in-r](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#fitting-linear-models-in-r){target="_blank"}
:::
...
::: {.callout-tip collapse="true"}
#### Hints
We saw how to extract lots of information on our model using `summary()` (see [Chapter 10 #model-summary](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#model-summary){target="_blank"}), but there are lots of other functions too.  
...
:::int
The estimated average time spent exploring novel objects for a monkey of age zero is `r round(coef(model1)[1],2)` minutes.
:::

For the estimated slope we get:

:::int
A one year increase in age is associated with an estimated decrease of `r round(coef(model1)[2],2)` minutes (or `r round(coef(model1)[2]*60)` seconds) that a monkey will spend exploring a novel object.  
:::
`r solend()`
...
See [Chapter 10 #inference-for-regression-coefficients](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#inference-for-regression-coefficients){target="_blank"}.
:::
...
:::int
A significant association was found between age (in years) and time spent exploring novel objects, with exploration time decreasing on average by `r round(coef(model1)[2],2)` minutes (or `r round(coef(model1)[2]*60)` seconds) for every additional year of age ($b = `r round(coef(model1)[2],2)`$, $SE = `r round(sqrt(diag(vcov(model1)))[2],3)`$, $t(`r model1$df.residual`)=-3.103$, $p=.003$). 
:::
...
<!-- - See [Ch 10 #r2](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#r2){target="_blank"} if you're unsure about what $R^2$ is.     -->
...
<!-- ::: {.callout-tip collapse="true"} -->
<!-- #### Hints -->
<!-- [Ch 10#the-f-statistic](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#the-f-statistic){target="_blank"} will help.   -->
<!-- ::: -->
...
::: {.callout-tip collapse="true"}
#### Hints
For our simple regression model, there is a really easy way to do this with `geom_smooth()`.  Check [Chapter 10 #example](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#example){target="_blank"}, which shows an example.
:::
...
::: {.callout-tip collapse="true"}
#### Hints
See [Chapter 10 #the-error](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#the-error){target="_blank"}.
:::
...
For any particular age, the time monkeys spend exploring novel objects is estimated as being distributed above and below the regression line with a standard deviation of $\hat \sigma = 4.32$. 
Since $1.96 \hat \sigma = 1.96 \times 4.32 = \text{approx} 8.6$, we would expect most (about 95\%) of the monkeys' exploration times to be within about 8.6 minutes from the regression line.
...
::: {.callout-tip collapse="true"}
#### Hints
Given that you know the intercept and slope, you can calculate this algebraically. However, try to also use the `predict()` function (see [Chapter 10 #model-predictions](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#model-predictions){target="_blank}). 
:::
...
`r qbegin(qcounter())`
Take a look at the assumption plots (see [Chapter 10 #assumptions](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#assumptions){target="_blank"}) for your model.  
...
::: {.callout-tip collapse="true"}
#### Hints
For help interpreting the coefficients, see [Chapter 10 #binary-predictors](https://uoepsy.github.io/usmr/2526/readings/10_slr.html#binary-predictors){target="_blank"}.  
:::
...
`r qbegin("EXTRA!", qlabel=FALSE)`

The data from this year's survey that you filled out in the first couple of weeks is available at [https://uoepsy.github.io/data/usmr2025.csv](https://uoepsy.github.io/data/usmr2025.csv)  

- **Step 1:** Using the USMR 2025 survey data, make *either* the prettiest plot you can or the ugliest plot you can . 
...
All plots submitted by end-of-day on 24th October will be rated by a number of non-USMR staff, and prizes made of chocolate will be awarded to the team with the prettiest plot and to the team with the ugliest plot. 
