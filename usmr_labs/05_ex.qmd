...
df = 
  ...
  mutate(
    age = round(runif(n(),1,21))
  )

df$exploration_time = as.matrix(cbind(1,df[,c(4,1,3)])) %*% c(14,-.27, 2, -1) + rnorm(nrow(df),0,3.5)
df$sex = sample(c("male","female"),nrow(df),T)
df$exploration_time = round(df$exploration_time[,1],1)
# df |> mutate(adult=ifelse(age>8,1,0)) |> pairs()

df <- df |> select(age,sex,obj_type,obj_colour, obj_size,exploration_time) |>
  mutate(
...
lm(exploration_time ~ age, df) |> summary()
lm(exploration_time ~ I(age>=8), df) |> summary()
lm(exploration_time ~ obj_type, df) |> summary()
lm(exploration_time ~ age + obj_type, df) |> plot(which=4)
lm(exploration_time ~ age + obj_type, df) |> summary()
lm(exploration_time ~ age + obj_type + obj_size + obj_colour, df) |> summary()
anova(
  lm(exploration_time ~ age + obj_type, df),
  lm(exploration_time ~ age + obj_type + obj_size + obj_colour, df)
)
lm(exploration_time ~ age + obj_type*sex, df) |> summary()
...
tibble(
  ...
  description = c("Age of monkey in years", "Sex", "Type of novel object given (soft / moving)","Main colour of object (red / green / blue)","Size of object in cm (length of largest dimension of the object)","Time (in minutes) spent exploring the object")
) |>
  gt::gt()
```
...
The plots suggests that the distributions of monkeys' ages and the time they spend exploring novel objects are both unimodal. Most of the monkeys are between roughly 5 and 15 years old, and most of them spent between 8 and 14 minutes exploring the objects. The boxplots suggest a couple of outliers the distribution of exploration-times, with 2 monkeys spending more than $1.5 \times IQR$ beyond the 3rd quartile.  
...
that is, $r_{\text{age, exploration-time}} = `r round(cor(monkeyexp[,c(1,6)])[1,2],2)`$
...
::: {.callout-tip collapse="true"}
#### Hints
Check [5B #example](05b_slr.html#example){target="_blank"}, which shows an example of using the __sjPlot__ package to create a plot.  
:::
...
`r qbegin("Optional: Question 13A", qlabel = FALSE)`
...
:::int
For any particular age, the time monkeys spend exploring novel objects should be distributed above and below the regression line with standard deviation estimated to be $\hat \sigma = 4.45$. 
Since $2 \hat \sigma = 2 (4.45) = 8.9$, we expect most (about 95\%) of the monkeys' exploration times to be within about 8.9 minutes from the regression line.
:::  

`r solend()`

`r qbegin("Optional: Question 13B", qlabel=FALSE)`
...
From the bottom right plot above, we can see that the 57th observation is looking a bit influential. There is also one datapoint that is looking weird in both of the left-hand plots, and again this is the same 57th monkey.  
Let's look at them:  
```{r}
monkeyexp[57, ]
```

This is a young monkey (only 1 year old), who seems to have spent quite a long time exploring the toy. It's important to remember that this monkey is a valuable datapoint, despite being a bit different from the general pattern.  

However, it would be nice to know how much this monkey is affecting our conclusions, so let's re-fit the model on everybody except that one monkey 
...
ggplot(monkeyexp, aes(x=age,y=exploration_time))+
  ...
  ylim(0,34) + 
  labs(title="With the monkey") +

ggplot(monkeyexp[-57,], aes(x=age,y=exploration_time))+
  geom_point()+
  geom_smooth(method=lm, fullrange=TRUE)+
  ylim(0,34) + 
  labs(title="Without the monkey")
```
...
Let's suppose instead of having measured monkeys' ages in years, researchers simply recorded whether each monkey was an adult or a juvenile (Rhesus Macaques reach adulthood at 8 years old).  
...
summary(model2)
```
- `(Intercept)` = the estimated exploration time of juvenile monkeys (12.96 minutes)
- `isAdultyes` = the estimated change in exploration time from juvenile monkeys to adult monkeys (-3.27 minutes)  
