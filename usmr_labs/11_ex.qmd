...
But really, in this specific case, where the interaction is just one coefficients, framing it as an F test doesn't give us anything more than just looking at the coefficients. Note that the p-value is the same for the interaction in the F test as it is for the t test for the coefficient.  

::: {.callout-tip collapse="true"}
#### so why bother with anova(mod) or anova(mod1,mod2)?  

In this case, there's not really much to gain.  
The two questions:

- Does the `weekstopped*health` interaction explain variance in happiness? 
- Is the `weekstopped*health` interaction coefficient different from zero?  

Are the same, because they are both just about one parameter.  
Model comparisons become really useful when we have multiple parameters that we want to test at the same time. 

As an example, suppose we had measured "health" as 3 groups, rather than a continuous variable. In that case, the two questions are slightly different, because there are two coefficients captured by the interaction:  

- Does the `weekstopped*healthcat` interaction explain variance in happiness? 
- **Which** of the `weekstopped*healthcat` interaction coefficient**s** is different from zero?  

E.g., if we had instead only 3 distinct groups of health:  
```{r}
tmp <- couchto5k |>
  mutate(
    # cut health into 3 groups:
    healthcat = cut(healthZ, 3)
  )

tmp_mod <- lm(happiness ~ age + season + week_stopped * healthcat, data = tmp)
