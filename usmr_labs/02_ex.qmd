...
We can also avoid ever using the standard deviation of our sample (`sd(usmr2022$sleeprating)`), and instead approximate the sampling distribution of the mean by "bootstrapping" - taking repeated _resamples_ with replacement from the original sample (see [Chapter 4 #standard-error](https://uoepsy.github.io/usmr/2526/readings/04_sampling.html#standard-error-in-practice){target="_blank"}.  
