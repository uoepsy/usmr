...
There was a section in reading [Chapter 1 #useful-settings](https://uoepsy.github.io/usmr/2627/readings/01_R.html#useful-settings){target="_blank"} that showed a couple of settings that are very useful:  

1. Avoiding saving/loading the workspace (this will keep you with a nice clean environment)
2. 'Wrapping' long lines of code (this will save you scrolling horizontally)

Follow the [the instructions in the reading](https://uoepsy.github.io/usmr/2627/readings/01_R.html#useful-settings){target="_blank"} to change these now. 

While you're at it - pick your favourite colour scheme for RStudio and change that too!

It might also be useful to find a font which differentiates 1 (the number) from l (the letter), and 0 (the number) from O (the letter).

::: {.callout-note collapse="true"}
#### I want my R to be in a certain language (Mac/Linux)

If you want, you can set R to be in a specific language.  
This can be useful to avoid issues with character encoding (i.e., ensuring special characters, accents, or non-English text display and save correctly)

1. Install the usethis package
2. In the console, type `usethis::edit_r_environ()` and hit enter. A file will open.  
3. In the file, set your preferred language. This will just be one line, and will be something like: `LANG=en_GB.UTF-8`
4. click save
5. close the file
6. in the top menu of RStudio, click Session > Restart R

:::
::: {.callout-note collapse="true"}
#### I want my R to be in a certain language (Windows)

If you want, you can set R to be in a specific language.  
This can be useful to avoid issues with character encoding (i.e., ensuring special characters, accents, or non-English text display and save correctly)

1. In the console, type `Sys.getlocale()` and see what languages are available. 
2. Install the usethis package
3. In the console, type `usethis::edit_r_profile()` and hit enter. A file will open.  
4. In the file, write a command to set your preferred locale from those available. This will just be one line, and will be something like: `Sys.setlocale("LC_ALL", "English_United Kingdom.utf8")`
5. click save
6. close the file
7. in the top menu of RStudio, click Session > Restart R


_If the language you want is not available in your locales (step 1), then open Windows Settings (Windows Key + I); Go to Time & Language > Language & region; Under Preferred languages, click "Add a language", and make sure it installs fully._   
...
Anonymised data are available at [https://uoepsy.github.io/data/usmr26survey_historical.csv](https://uoepsy.github.io/data/usmr26survey_historical.csv)  
...
surveydata <- read_csv("https://uoepsy.github.io/data/usmr26survey_historical.csv")
...
`data <- data |> mutate(...)`  
(see [Chapter 3 #advances-in-r](https://uoepsy.github.io/usmr/2627/readings/03_measurement.html#advances-in-r){target="_blank"})
...
- See also [Chapter 3 #categorical](https://uoepsy.github.io/usmr/2627/readings/03_measurement.html#categorical){target="_blank"}.  
...
- We're likely to have missing data in here, so `na.rm=TRUE` will be handy (see [Chapter 3 #numeric](https://uoepsy.github.io/usmr/2627/readings/03_measurement.html#numeric){target="_blank"})
...
::: {.callout-tip collapse="true"}
#### Hints  
if we want a histogram, then `hist()` won't cut it here, we're going to want to use **ggplot** to make a lovely pretty histogram, (ggplot was introduced in [Chapter 3 #ggplot](https://uoepsy.github.io/usmr/2627/readings/03_measurement.html#ggplot){target="_blank"}).  
:::
`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`
```{r}
...
Note that the USMR students have a very slightly bigger standard deviation. So this plot will be slightly more spread out. It might be hard to see these differences just by looking at the plots though.  
...
```{r}
sum(surveydata$height > 176, na.rm = T)
```
...
```{r}
sum(surveydata$height > 176, na.rm = T) / sum(!is.na(surveydata$height))
```
...
surveydata |> 
  summarise(
    n_taller = sum(height > 176, na.rm = T),
...
::: {.callout-tip collapse="true"}
#### Hints  
This will need some `filter`ing, and assigning (e.g. `usmr2022on <-`) to a new name.
:::
`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`
```{r}
usmr2022on <- surveydata |> 
...
::: {.callout-tip collapse="true"}
#### Hints  
You can do this with things like `mean(data$variable)`, or you can do it all in tidyverse (see the example of `summarise` in the intro to tidyverse: [Chapter 3 #advances-in-r](https://uoepsy.github.io/usmr/2627/readings/03_measurement.html#advances-in-r){target="_blank"}).  
...
- The previous question gives you all the pieces that you need. You'll just need to put them together in the way seen in [Chapter 4 #confidence-intervals](https://uoepsy.github.io/usmr/2627/readings/04_sampling.html#confidence-intervals){target="_blank"}.  
...
```{r}
#| echo: false
mu = mean(usmr2022on$sleeprating, na.rm = TRUE)
su = sd(usmr2022on$sleeprating, na.rm = TRUE)
m = round(mean(usmr2022on$sleeprating, na.rm = TRUE),2)
s = round(sd(usmr2022on$sleeprating, na.rm = TRUE),2)
n = sum(!is.na(usmr2022on$sleeprating))
se = round(su/sqrt(n),2)
```

So we have our estimate, which is the mean sleep quality rating in our sample: $\bar x = `r m`$. And we have a standard deviation of our sleep quality ratings: $s = `r s`$. While our sample has `r nrow(usmr2022on)` students in it, only `r n` responded to the sleep question, so really our $n$ for this calculation is `r n`.  
...
We can also avoid ever using the standard deviation of our sample (`sd(usmr2022on$sleeprating)`), and instead approximate the sampling distribution of the mean by "bootstrapping" - taking repeated _resamples_ with replacement from the original sample (see [Chapter 4 #standard-error](https://uoepsy.github.io/usmr/2627/readings/04_sampling.html#standard-error-in-practice){target="_blank"}.  
...
For 3, look up `quantile()`. We saw this in [Chapter 4 #confidence-intervals](https://uoepsy.github.io/usmr/2627/readings/04_sampling.html#confidence-intervals){target="_blank"}.  
