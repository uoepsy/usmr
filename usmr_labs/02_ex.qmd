...
We can also avoid ever using the standard deviation of our sample (`sd(usmr2022$sleeprating)`), and instead approximate the sampling distribution of the mean by "bootstrapping" - taking repeated _resamples_ with replacement from the original sample. TODO link.  
...
2. Plot a histogram of the bootstrap means
3. What is the standard deviation of the bootstrap distribution? how does it compare to the standard error you calculated in the previous question with the formula?  
4. At what values does the middle 95% of the bootstrap distribution fall?  


::: {.callout-tip collapse="true"}
#### Hints

For 4, look up `quantile()`. We saw this in [2B #confidence-intervals](02b_sampling.html#confidence-intervals){target="_blank"}.  
...
3. Here's the standard deviation. Recall our standard error calculated using $\frac{s}{\sqrt{n}}$ from the previous question was `r se`
```{r}
sd(BSmeans)
```


4. And we can get the 2.5% and 97.5% percentiles (i.e. getting the middle 95%), using the code below. Recall our confidence intervals that we computed analytically were `r round(m-(1.96*se),2)` and `r round(m+(1.96*se),2)`.  
```{r}
