...
:::sticky
__scaling/standardising__  

Scaling changes the _units_ of the variable, and we do this by dividing the observations by some value. E.g., moving from "36 months" to "3 years" involves scaling by 12.  

Far and away the most common sort of scaling is 'standardisation'.  
...
standardised coefficients are dependent upon sample variances 

```{r}
df1 <- tibble(
  age = round(runif(100,18,40)),
  vocabulary = age*1 + rnorm(100,0,3)
)
df2 <- tibble(
  age = round(runif(100,18,80)),
  vocabulary = age*1 + rnorm(100,0,3)
)
lm(scale(vocabulary) ~ scale(age), data = df1) |> 
  summary()
lm(scale(vocabulary) ~ scale(age), data = df2) |> 
  summary()
