...
`r qbegin("A1")`
In the survey during the welcome week, we asked you which month you were born in.  
...
```{r}
survey_data <- read_csv("https://uoepsy.github.io/usmr/surveyresults//usmr_surveyresponse.csv") 
```
The quick and dirty way:
```{r}
barplot(table(survey_data$birthdaymonth))
```

A ggplot option:
```{r}
ggplot(data = survey_data, aes(x = birthdaymonth)) +
    geom_bar() +
    labs(x = "- Birth Month -")
```
`r solend()`
...
`r qbegin("A3")`
Given that there `r nrow(survey_data)` observations in our sample, how many observations would we *expect* to find with a birthday in January? And in February? ... and so on? 
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
```
(A shortcut for this would be `survey_data %>% count(birthdaymonth)`)  
...
2. Check that these results match with those provided by R's built-in function: `chisq.test(table(survey_data$birthdaymonth))`.  
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r}
sum(chi_table$std_sq_diff)
...
One possible way to answer this question is to look at the individual contribution of each category to the chi-square statistic. We computed these values in Question A4.  
```{r}
chi_table %>%
  select(birthdaymonth, std_sq_diff)
```

From the barplot we created earlier on, we can see which months make up higher/lower proportions than expected:
```{r}
ggplot(chi_table, aes(x = birthdaymonth, y = observed/nrow(survey_data))) +
...
```{r}
chisq.test(table(survey_data$birthdaymonth))$residuals
```

```{r echo=FALSE}
presids <- chisq.test(table(survey_data$birthdaymonth))$residuals
...
Perform a chi-squared goodness of fit test to assess the extent to which USMR students of 2020/21 conform to this theorised distribution of eye-colours.  
...
```{r}
chisq.test(table(survey_data$eyecolour), p = c(.05,.1,.76,.01,.02,.05,.01))
```
`r solend()`
...
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
In total, 60 customers tipped (14+30+16), and 151 did not. So *overall*, 0.4 ($\frac{60}{151}$) of customers tip.  
...
So we would expect 0.4 of each group to leave a tip.  
...
Degrees of freedom for a chi-square test of independence is calculated as:  
$$
\begin{align}
& df = (r - 1)(c - 1) \\
& \text{Where:} \\
...
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
While the fertiliser does improve plant growth to a statistically significant (at $\alpha=0.05$) degree, the improvement is minimal. The difference in means is only 1.2737, which is about 1/4 of a tomato.  
